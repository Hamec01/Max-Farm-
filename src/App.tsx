...
import { INITIAL_STATE, ANIMAL_TEMPLATES, CROPS_CONFIG, TREES_CONFIG, LOCATIONS, UPGRADES, BUILDINGS_TEMPLATES } from "./data";
...
export default function App() {
  ...
  const [gameState, setGameState] = useState<PlayerState>(() => {
    ...
    if (saved) {
      try {
        ...
        if (!parsed.trees) parsed.trees = INITIAL_STATE.trees;
        if (!parsed.workers || parsed.workers.length === 0) parsed.workers = INITIAL_STATE.workers;
        if (parsed.day === undefined) parsed.day = 1;
        if (parsed.dayProgress === undefined) parsed.dayProgress = 0;
        return parsed;
      } catch (e) {
  ...
  // Kid-friendly Room/Room Transition helpers
  const locationKeys: LocationId[] = ["MEADOW", "BARNYARD", "LAKESIDE", "ORCHARD"];
  const currentIndex = locationKeys.indexOf(activeZone);
  const prevLocation = currentIndex > 0 ? locationKeys[currentIndex - 1] : null;
  const nextLocation = currentIndex < locationKeys.length - 1 ? locationKeys[currentIndex + 1] : null;
  ...
  // Sync mute state with our audio module
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setGameState((prev) => {
        ...
        const isFeedHired = updatedWorkers.find(w => w.id === "worker-feed")?.isActive;
        const isGrowHired = updatedWorkers.find(w => w.id === "worker-grow")?.isActive;
        const isCleanHired = updatedWorkers.find(w => w.id === "worker-clean")?.isActive;
            ...
            if (!animal.isFed && !hasFedOne) {
              ...
              if (hasFood) {
                ...
                setTimeout(() => {
                  ...
                  triggerWorkerActionFeedback("worker-feed", animal.x, animal.y, `🍿 Кормлю ${config.nameRu.split(" ")[0]}!`);
                }, 40);
          ...
          Object.keys(updatedCrops).forEach((plotId) => {
            ...
            if (crop && crop.progress > 0 && crop.progress < 100 && !crop.isWatered && !waterCheck) {
              ...
              setTimeout(() => {
                ...
                if (coords) {
                  triggerWorkerActionFeedback("worker-grow", coords.x, coords.y, "💧 Поливаю!");
                }
              }, 40);
            }
          });

          // B2. Re-plants seeds or plants crops suitable for current animals' diet
          let plantCheck = false;
          Object.keys(updatedCrops).forEach((plotId) => {
            const crop = updatedCrops[plotId];
            if (crop && crop.progress === 0 && !plantCheck) {
              // Collect the food types needed by the active animals
              const farmAnimals = prev.animals || [];
              const neededFoods = new Set<string>();
              farmAnimals.forEach(animal => {
                const t = ANIMAL_TEMPLATES[animal.species];
                if (t && t.foodType) {
                  neededFoods.add(t.foodType);
                }
              });

              const seedKeys = ["WHEAT_SEED", "CARROT_SEED", "CLOVER_SEED", "CABBAGE_SEED"];
              const availableSeedsInStock = seedKeys.filter(key => (updatedInventory[key] || 0) > 0);

              let seedToPlant: string | null = null;
              if (availableSeedsInStock.length > 0) {
                // Try to find a seed whose crop matches animal diet needs
                const matchingSeed = availableSeedsInStock.find(seed => {
                  const cropType = seed.replace("_SEED", "");
                  return neededFoods.has(cropType);
                });
                seedToPlant = matchingSeed || availableSeedsInStock[0];
              }

              if (seedToPlant) {
                const cropType = seedToPlant.replace("_SEED", "") as CropType;
                updatedCrops[plotId] = {
                  id: plotId,
                  type: cropType,
                  progress: 2,
                  isWatered: true,
                  isDead: false,
                  timeRemaining: CROPS_CONFIG[cropType].growTime
                };
                updatedInventory[seedToPlant]--;
                plantCheck = true;
                setTimeout(() => {
                  spawnFloatHeart(50, 42, "🌱");
                  const coords = getPlotsCoords(plotId);
                  if (coords) {
                    triggerWorkerActionFeedback("worker-grow", coords.x, coords.y, `🌱 Сею ${CROPS_CONFIG[cropType].nameRu}!`);
                  }
                }, 45);
              } else {
                // Autobuy appropriate seed!
                const choices: { crop: CropType, seed: string, cost: number }[] = [
                  { crop: "WHEAT", seed: "WHEAT_SEED", cost: 5 },
                  { crop: "CARROT", seed: "CARROT_SEED", cost: 10 },
                  { crop: "CLOVER", seed: "CLOVER_SEED", cost: 18 },
                  { crop: "CABBAGE", seed: "CABBAGE_SEED", cost: 28 }
                ];

                // Filter choices that match animal diets:
                const neededChoices = choices.filter(choice => neededFoods.has(choice.crop));
                
                let chosenToBuy = neededChoices.find(choice => nextCoins >= choice.cost);
                if (!chosenToBuy) {
                  // Fallback to any choice matches animal diet, or default to wheat
                  chosenToBuy = choices.find(choice => nextCoins >= choice.cost);
                }

                if (chosenToBuy) {
                  updatedCrops[plotId] = {
                    id: plotId,
                    type: chosenToBuy.crop,
                    progress: 2,
                    isWatered: true,
                    isDead: false,
                    timeRemaining: CROPS_CONFIG[chosenToBuy.crop].growTime
                  };
                  nextCoins -= chosenToBuy.cost;
                  plantCheck = true;
                  setTimeout(() => {
                    spawnFloatHeart(50, 42, "🌱");
                    const coords = getPlotsCoords(plotId);
                    if (coords) {
                      triggerWorkerActionFeedback("worker-grow", coords.x, coords.y, `🌱 Сею ${CROPS_CONFIG[chosenToBuy.crop].nameRu}!`);
                    }
                  }, 45);
                }
              }
          ...
          Object.keys(updatedCrops).forEach((plotId) => {
            ...
            if (crop && crop.progress >= 100 && !crop.isDead && !harvestCheck) {
              ...
              setTimeout(() => {
                ...
                const coords = getPlotsCoords(plotId);
                if (coords) {
                  triggerWorkerActionFeedback("worker-grow", coords.x, coords.y, `🧺 Сбор ${config.nameRu}!`);
                }
              }, 45);
            }
          });
        ...
        if (isCleanHired) {
          ...
          updatedAnimals = updatedAnimals.map((animal) => {
            if (animal.productionProgress >= 100 && !collectCheck) {
              const config = ANIMAL_TEMPLATES[animal.species];
              ...
              setTimeout(() => {
                ...
                triggerWorkerActionFeedback("worker-clean", animal.x, animal.y, `🧺 Сбор у ${config.nameRu.split(" ")[0]}!`);
              }, 50);
              return {
                ...animal,
          ...
          Object.keys(updatedTrees).forEach((treeId) => {
            ...
            if (tree && tree.fruitCount > 0 && !fruitCheck) {
              ...
              setTimeout(() => {
                ...
                if (coords) {
                  triggerWorkerActionFeedback("worker-clean", coords.x, coords.y, `🍎 Сбор ${config.fruitNameRu}!`);
                }
              }, 50);
            }
          });

          // C3. Brushes / Grooms 1 dirty or unhappy animal
          ...
          updatedAnimals = updatedAnimals.map((animal) => {
            if ((animal.cleanliness < 70 || animal.happiness < 75) && !brushCheck) {
              ...
              setTimeout(() => {
                spawnFloatHeart(animal.x, animal.y, "🧼");
                triggerWorkerActionFeedback("worker-clean", animal.x, animal.y, `🧼 Чищу ${ANIMAL_TEMPLATES[animal.species].nameRu.split(" ")[0]}!`);
              }, 40);
              return {
                ...animal,
                cleanliness: 100,
        ...
        Object.keys(next).forEach((wid) => {
          ...
          if (dist < 0.8) {
            ...
            if (shouldPickNewTarget && currentActionTimer === 0) {
              isMoving = true;
              if (wid === "worker-papa" || wid === "worker-pasha") {
                targetX = 20 + Math.random() * 50;
                targetY = 60 + Math.random() * 20;
              } else if (wid === "worker-mama" || wid === "worker-nadya" || wid === "worker-lena") {
                targetX = 15 + Math.random() * 65;
                targetY = 62 + Math.random() * 18;
              } else if (wid === "worker-andrey") {
                targetX = 15 + Math.random() * 70;
                targetY = 56 + Math.random() * 24;
              } else if (wid === "worker-dima" || wid === "worker-arina") {
  ...
  const getAnimalsInZone = (zone: LocationId) => {
    return gameState.animals.filter((animal) => {
      ...
      if (zone === "MEADOW") {
        return [AnimalSpecies.CHICKEN, AnimalSpecies.DUCK, AnimalSpecies.GOOSE, AnimalSpecies.TURKEY, AnimalSpecies.RABBIT, AnimalSpecies.SHEEP, AnimalSpecies.CAT, AnimalSpecies.DOG].includes(sp);
      }
      if (zone === "BARNYARD") {
  ...
  const handleNextZone = () => {
    ...
    const currentIndex = ZONES_ORDER.indexOf(activeZone);
    const nextIndex = (currentIndex + 1) % ZONES_ORDER.length;
  ...
  const handlePrevZone = () => {
    const ZONES_ORDER: LocationId[] = ["MEADOW", "BARNYARD", "LAKESIDE", "ORCHARD", "DESERT", "FOREST", "LAKE"];
    const currentIndex = ZONES_ORDER.indexOf(activeZone);
    const prevIndex = (currentIndex - 1 + ZONES_ORDER.length) % ZONES_ORDER.length;
  ...
  return (
    <div className="min-h-screen bg-[#FFFEEF] flex flex-col font-sans transition-all duration-500 pb-12 overflow-x-hidden" id="root-viewport-game">
      {/* HUD Header bar has been integrated directly "above the sky" inside the pasture viewport below! */}

      <main className="w-full max-w-none mt-0 flex flex-col gap-4 animate-fade-in" id="main-farm-container">
        
        {/* WALKING WORLD VIEWPORT CANVAS - FULL BLEED RESIZING FOR ALL SCREENS */}
        <div className="relative w-full shadow-lg" id="playground-viewport-wrapper">
          <div
            ...
            className={`h-[500px] xs:h-[580px] sm:h-[660px] md:h-[720px] lg:h-[760px] rounded-none sm:rounded-3xl relative overflow-hidden transition-all duration-[1000ms] select-none ${
              isNight
          ...
          >
            ...
            <div className="absolute left-4 right-4 bottom-2.5 z-30 flex items-center justify-between gap-4 pointer-events-none select-none">
              {/* Miniature Cartoon Backpack containing non-empty items (No letters, only images/emojis and count figures) */}
              <div className="flex items-center gap-1.5 bg-[#FFF8DF]/95 border-2 border-[#7A4E31] p-1 px-2.5 rounded-full shadow-md pointer-events-auto max-w-[calc(100%-110px)] overflow-x-auto scrollbar-none" id="compact-backpack-hud">
                <span className="text-sm select-none" title="Твой Рюкзак">🎒</span>
              ...
              {activeZone !== "BARNYARD" && (
                <button
                  ...
                  className="bg-gradient-to-r from-amber-500 to-yellow-400 hover:from-amber-600 hover:to-yellow-500 text-white rounded-full border-2 border-[#7A4E31] shadow-md flex items-center justify-center gap-1 active:scale-95 transform transition-all p-1 px-3 pointer-events-auto cursor-pointer select-none font-sans font-black text-[9.5px] uppercase tracking-wider shrink-0 leading-none"
            ...
            {gameState.workers.map((worker) => {
              ...
              } else if (worker.id === "worker-clean") {
                actionIcon = "🧹";
                workerSVG = (
                  <svg viewBox="0 0 60 70" className="w-16 h-18 filter drop-shadow-md">
                    {/* Cap worn backwards */}
            ...
            {selectedPlotId && (() => {
              ...
              return (
                <div
                  className="absolute bg-white border-4 border-[#92400E] rounded-3xl p-3 shadow-2xl flex flex-col items-center text-center gap-2 z-30 select-none animate-scale-up"
                  style={{
                    left: `${coords.x}%`,
                    top: `${coords.y - 12}%`,
                ...
                >
                  <span className="text-xs bg-amber-950 text-white font-black px-2 py-0.5 rounded-full scale-90 block">
            ...
            {selectedTreeId && (() => {
              ...
              return (
                <div
                  className="absolute bg-white border-4 border-emerald-700 rounded-3xl p-3 shadow-2xl flex flex-col items-center text-center gap-2 z-30 select-none animate-scale-up"
                  style={{
                    left: `${coords.x}%`,
                    top: `${coords.y - 14}%`,
                    transform: 'translateX(-50%) translateY(-50%)',
                ...
                >
                  <span className="text-xs bg-emerald-700 text-white font-black px-2.5 py-0.5 rounded-full scale-90 block">
                    🌳 ПЛОДОВАЯ ЛУНКА 🍎
                  </span>
            ...
            {selectedAnimalId && (() => {
              ...
              return (
                <div
                  className="absolute bg-white border-4 border-[#92400E] rounded-[24px] p-2.5 shadow-2xl flex flex-col items-center text-center gap-1.5 z-30 select-none animate-scale-up"
                  style={{
                    left: `${animal.x}%`,
                    top: `${animal.y - 15}%`,
                    transform: 'translateX(-50%) translateY(-50%)',
                ...
                >
                  <div className="text-xs lg:text-sm font-sans font-black text-slate-800 leading-3">
                    {animal.customName} {template.emoji}
                  </div>
                  <div className="text-[10px] text-slate-500 font-extrabold uppercase bg-amber-50 rounded px-1">{template.nameRu}</div>
                  ...
                  {!isNear ? (
                    <button
                      ...
                      className="mt-1 py-2 px-4 bg-[#D97706] hover:bg-[#B45309] text-white text-xs font-black rounded-full cursor-pointer uppercase flex items-center justify-center gap-1"
                    >
                    ...
                    <div className="w-full space-y-1 px-1">
                      {/* STATS */}
                      <div className="flex justify-between text-[10px] text-slate-600 font-black border-b pb-0.5 mb-1 bg-slate-50 p-1 rounded">
                      ...
                      <button
                        ...
                        className="w-full py-2 bg-yellow-400 hover:bg-yellow-500 text-[#402315] text-xs font-black rounded-lg cursor-pointer flex items-center justify-center gap-1 uppercase border-b-2 border-yellow-600"
                      >
                      ...
                      <button
                        ...
                        className="w-full py-2 bg-blue-400 hover:bg-blue-500 text-white text-xs font-black rounded-lg cursor-pointer flex items-center justify-center gap-1 uppercase border-b-2 border-blue-600"
                      >
                        🧼 Погладить & щетка
                      </button>
                      ...
                      {animal.productionProgress >= 100 && (
                        animal.species === AnimalSpecies.SHEEP ? (
                          <button
                            ...
                            className="w-full py-2 bg-pink-500 hover:bg-pink-600 text-white text-xs font-bold rounded-lg cursor-pointer flex items-center justify-center gap-1 uppercase animate-pulse shadow-sm border-b-2 border-pink-700"
                          >
                            ✂️ Постричь шерсть!
                          </button>
                          ...
                          <button
                            ...
                            className="w-full py-2 bg-cyan-500 hover:bg-cyan-600 text-white text-xs font-bold rounded-lg cursor-pointer flex items-center justify-center gap-1 uppercase animate-pulse shadow-sm border-b-2 border-cyan-700"
                          >
                            🥛 Подоить молоко!
                          </button>
                        ) : (
                          <button
                            ...
                            className="w-full py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold rounded-lg cursor-pointer flex items-center justify-center gap-1 uppercase animate-pulse shadow-sm border-b-2 border-emerald-700"
                          >
                      ...
                      <button
                        ...
                        className="w-full py-1 mt-1 text-[10px] bg-red-50 text-red-700 hover:bg-red-100 font-extrabold rounded-md cursor-pointer uppercase block border border-red-200"
                      >
                        🍂 Отпустить погулять
                      </button>
                    </div>
            ...
            {/* --- MULTI-ROOM / LEVEL NAVIGATION ARROWS --- */}
            {prevLocation && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  playClickSound();
                  setActiveZone(prevLocation);
                  setBoyPosition({ x: 80, y: 70, targetX: 80, targetY: 70, isMoving: false, dir: "left" });
                  setSelectedAnimalId(null);
                  setSelectedPlotId(null);
                  setSelectedTreeId(null);
                  triggerNotification(`🚪 Вошли в: ${LOCATIONS[prevLocation].nameRu}!`);
                }}
                className="absolute left-3 top-[50%] transform -translate-y-1/2 bg-gradient-to-b from-[#FEF3C7] to-[#FDE68A] hover:from-white hover:to-amber-50 border-4 border-[#7A4E31] p-2 px-2.5 rounded-[20px] flex flex-col items-center gap-0.5 shadow-2xl hover:scale-110 active:scale-95 transition-all duration-200 z-30 cursor-pointer"
              ...
              >
                ...
                <span className="text-[7.5px] font-extrabold text-[#92400E] max-w-[55px] truncate leading-none mt-0.5 text-center">{LOCATIONS[prevLocation].nameRu}</span>
              </button>
            )}

            {nextLocation && (() => {
              const isUnlocked = gameState.unlockedLocations.includes(nextLocation);
              const nextConfig = LOCATIONS[nextLocation];
              return (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    playClickSound();
                    if (isUnlocked) {
                      setActiveZone(nextLocation);
                      setBoyPosition({ x: 20, y: 70, targetX: 20, targetY: 70, isMoving: false, dir: "right" });
                      setSelectedAnimalId(null);
                      setSelectedPlotId(null);
                      setSelectedTreeId(null);
                      triggerNotification(`🚪 Вошли в: ${LOCATIONS[nextLocation].nameRu}!`);
                    } else {
                      playSadSound();
                      triggerNotification(`🔒 Ой! Комната закрыта. Сними замок на рынке за ${nextConfig.unlockCost} золотых на Уровне ${nextConfig.minLevel}!`);
                      setShopActiveTab("lands");
                      setShowShopModal(true);
                    }
                  }}
                  className={`absolute right-3 top-[50%] transform -translate-y-1/2 p-2 px-2.5 rounded-[20px] flex flex-col items-center gap-0.5 shadow-2xl hover:scale-110 active:scale-95 transition-all duration-200 z-30 border-4 cursor-pointer ${
                    isUnlocked 
                ...
                >
                  <span className="text-xl">{isUnlocked ? "➡️" : "🔒"}</span>
      ...
      {showShopModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4 animate-fade-in" id="shop-modal-panel">
          <div className="bg-[#FFFCEF] border-8 border-[#6B3F23] rounded-[42px] p-6 shadow-2xl max-w-2xl w-full relative max-h-[92vh] overflow-y-auto animate-scale-up" id="merchant-nearby-drawer">
            {/* Close Shop Button */}
            <button
              onClick={() => { playClickSound(); setShowShopModal(false); }}
              className="absolute top-4 right-4 w-10 h-10 bg-white hover:bg-rose-50 text-rose-600 border-4 border-[#6B3F23] transition rounded-full cursor-pointer shadow-md font-black text-sm flex items-center justify-center"
            >
              ❌
            </button>

            {/* Shop Header */}
            <div className="text-center mb-4 border-b-4 border-dashed border-[#6B3F23]/25 pb-3">
              <span className="text-5xl select-none">🏪</span>
              <h2 className="text-2xl font-black text-[#6B3F23] mt-1">Рынок купца Семена</h2>
              <p className="text-xs text-amber-800 font-bold">Веселый и мирный обмен без забоя животных! 🌸</p>
            </div>

             {/* Central big category tabs */}
            <div className="flex flex-wrap gap-1.5 justify-center mb-4 bg-[#6B3F23]/10 p-1.5 rounded-2xl" id="shop-tabs">
              {(["sell", "animals", "upgrades", "lands", "workers"] as const).map((tab) => (
                <button
                  key={tab}
                  onClick={() => { playClickSound(); setShopActiveTab(tab); }}
                  className={`flex-1 min-w-[90px] py-2 px-3 text-center text-xs font-black rounded-xl uppercase transition cursor-pointer border-2 ${
                    shopActiveTab === tab
            ...
            {shopActiveTab === "sell" && (
              <div className="space-y-4">
                ...
                <div className="bg-yellow-400/90 border-4 border-yellow-600 rounded-[24px] p-4 text-center shadow-lg transform hover:scale-102 transition-transform">
                  <span className="text-4xl block animate-bounce-slow">🪙</span>
                  <p className="text-xs text-amber-950 font-black mt-1 uppercase tracking-wide">Самая крутая кнопка на ферме:</p>
                  <button
                    onClick={handleSellAllProducts}
                    className="mt-2 py-3 px-8 rounded-2xl bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-white font-black text-sm border-b-4 border-orange-700 shadow-md active:translate-y-0.5 cursor-pointer uppercase w-full sm:w-auto"
                  >
                    💰 ПРОДАТЬ ВСЁ НАШЕ ДОБРО! 💰
                  </button>
                  <p className="text-[10px] text-amber-900 font-bold mt-2">Купец Семен выкупит весь сорванный урожай, молоко и шерстку одной кнопкой!</p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[220px] overflow-y-auto pr-1">
                  {Object.keys(gameState.inventory).filter(k => !k.endsWith("_SEED")).map((key) => {
                    ...
                    return (
                      <div key={key} className="bg-white p-2.5 rounded-2xl border-2 border-[#6B3F23]/15 flex items-center justify-between shadow-xs">
                        <div className="flex items-center gap-2">
                          <span className="text-3xl select-none">{icon}</span>
                          <div>
                            <h4 className="font-black text-xs text-[#6B3F23]">{nameRu}</h4>
                            <p className="text-[9px] text-gray-500 font-black">У тебя в заначе: {count} шт.</p>
                          </div>
                        </div>
                        <button
                          onClick={() => handleSellProductGroup(key, count)}
                          className="py-1 px-3 bg-[#FEF3C7] hover:bg-[#FEF9E7] border border-[#D97706]/40 rounded-xl text-[9.5px] font-black text-[#92400E] cursor-pointer"
                        >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5 max-h-[360px] overflow-y-auto pr-1">
                {(Object.keys(ANIMAL_TEMPLATES) as AnimalSpecies[]).map((species) => {
                  ...
                  return (
                    <div
                      key={species}
                      className={`bg-white p-3 rounded-3xl border-4 flex flex-col justify-between text-center transition-all shadow-sm ${
              ...
              <div className="space-y-4 max-h-[380px] overflow-y-auto pr-1">
                ...
                <div className="space-y-2">
                  {Object.keys(UPGRADES).map((id) => {
                    ...
                    return (
                      <div key={id} className="bg-white p-3 rounded-2xl border-2 border-[#6B3F23]/15 flex items-center justify-between shadow-xs">
                        <div className="flex items-center gap-2.5">
                          <span className="text-3xl bg-amber-50 p-1.5 rounded-xl border border-amber-100 select-none">{upgrade.icon}</span>
                          <div>
            ...
            {shopActiveTab === "lands" && (
              <div className="space-y-4 max-h-[380px] overflow-y-auto pr-1">
                ...
                {Object.keys(LOCATIONS).map((locId) => {
                  ...
                  return (
                    <div key={locId} className={`bg-white p-3 rounded-2xl border-2 flex flex-col gap-3 shadow-xs ${isUnlocked ? "border-green-400 bg-green-50/10" : "border-[#6B3F23]/15"}`}>
                      {/* FIRST ROW: LAND LOCATION DETAILS */}
                      <div className="flex items-center justify-between gap-2.5">
                        <div className="flex items-center gap-2.5">
                          <span className="text-3xl bg-[#FEF3C7] p-2 rounded-xl h-12 w-12 flex items-center justify-center">
                            {locId === "MEADOW" && "🌸"}
                            {locId === "BARNYARD" && "🏡"}
                            {locId === "LAKESIDE" && "🦆"}
                            {locId === "ORCHARD" && "🌳"}
            ...
            {shopActiveTab === "workers" && (
              <div className="space-y-3.5 max-h-[380px] overflow-y-auto pr-1">
                ...
                {(gameState.workers || []).map((worker) => {
                  ...
                  return (
                    <div key={worker.id} className={`bg-white p-3 rounded-2xl border-2 flex flex-col sm:flex-row items-center justify-between shadow-xs ${worker.isActive ? "border-green-400 bg-green-50/10" : "border-[#6B3F23]/15"}`}>
                      <div className="flex items-center gap-3 w-full sm:w-auto">
                        <span className={`text-4xl p-2.5 rounded-2xl ${worker.color}`}>
                          {worker.emoji}
                        </span>
                        <div className="text-left">
                          ...
                          <p className="text-[9px] text-slate-500 leading-3.5 mt-1.5 max-w-[340px]">
                            {worker.id === "worker-feed" && "Кормит голодных зверушек, если в инвентаре есть пшеница, капуста, морковка или молоко. Ускоряет рост продукции!"}
                            {worker.id === "worker-grow" && "Поливает сухие грядки, высаживает пшеницу (или покупает зерно по 5 монет) и собирает спелые колосья!"}
                            {worker.id === "worker-clean" && "Моет испачканных зверят, собирает их продукты (яйца, шерсть, коровье молоко) и собирает спелые яблоки с вишней!"}
                          </p>
            ...
            <div className="mt-4 pt-3.5 border-t-4 border-dashed border-[#6B3F23]/15 flex justify-between items-center text-[10px] text-[#6B3F23] font-black uppercase tracking-wider">
      ...
      {customNotification && (
        <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-[#FFFBEB] text-[#92400E] text-xs md:text-sm font-black p-3.5 px-6 rounded-3xl shadow-2xl border-4 border-[#92400E] z-50 animate-bounce-slow flex items-center gap-2" id="live-notification">
